...
**CRITICAL:** Output *only* JSON. No markdown fences, no commentary.

### File Output Convention (important)
The generator will write files based on your codeSnippets[].filename values.
- If you provide a bare filename (e.g. main.ino), it will be written under code/.
- You may also provide explicit paths like code/main.ino, docs/overview.md, schematics/diagram.mmd.

Target package layout:
- code/
- schematics/
- docs/
- bom.csv (generated from the bom array)
- report.md (generated)
- prototype.json (raw JSON you return)
...

{
  ...
  "threeDDescription": {
    "enclosure": "Description of enclosure",
    "mounting": "Mounting details"
  },
  "threeDHtml": "A full standalone HTML document (no external build step) that previews the 3D concept using Three.js from a CDN. Keep it lightweight. The web UI will render this as the 3D preview.",
